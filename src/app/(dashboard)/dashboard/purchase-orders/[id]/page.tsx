  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <div className="mb-1"><Link href="/dashboard/purchase-orders" className="text-sm text-muted-foreground hover:underline">← Purchase Orders</Link></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="text-sm space-y-1">
            <p className="font-medium">{doc.supplier?.companyName}</p>
            <p className="text-muted-foreground">{doc.supplier?.name}</p>
            {doc.supplier?.email && <p>{doc.supplier.email}</p>}
          </CardContent>
        </Card>
        <Card>
