...
const columns: Column<ClientRow>[] = [
  { key: "companyName", header: "Company", sortable: true, render: r => (
    <Link href={`/dashboard/clients/${r.id}`} className="font-medium hover:underline">{r.companyName}</Link>
  )},
  ...
  { key: "actions", header: "", render: r => (
    <Link href={`/dashboard/clients/${r.id}`}><Button variant="outline" size="sm">View</Button></Link>
  )},
];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/dashboard/clients/new"><Button>+ New Client</Button></Link>
      </div>
      <DataTable data={rows} columns={columns} searchPlaceholder="Search clients..." searchFields={["companyName", "name", "email"]} />
    </div>
  );
}
